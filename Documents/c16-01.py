import csv
import smtplib
from email.mime.multipart import MIMEMultipart
from email.mime.text import MIMEText
from email.mime.base import MIMEBase
from email import encoders
# import pdfkit
from email.mime.multipart import MIMEMultipart
from email.mime.text import MIMEText
import datetime


class Combiner:

    def email_sender(self, msghtml):
        fromaddr = "dairystash@gmail.com"
        email_list = ['manny@dairyperformancenetwork.com',
                      'gilberto@dpnconnect.com',
                      'support@dairyperformancenetwork.com',
                      'redrockdairy92@gmail.com']

        for email in email_list:
            toaddr = email

            # instance of MIMEMultipart
            msg = MIMEMultipart()

            # storing the senders email address
            msg['From'] = fromaddr

            # storing the receivers email address
            msg['To'] = toaddr

            # storing the subject
            msg['Subject'] = "Red Rock Weekly Repro Report"

            # string to store the body of the mail
            body = msghtml

            # attach the body with the msg instance
            msg.attach(MIMEText(body, 'html'))

            # open the file to be sent
            filename = "Download Pdf File From Here.pdf"
            attachment = open("output.pdf", "rb")

            # instance of MIMEBase and named as p
            p = MIMEBase('application', 'octet-stream')

            # To change the payload into encoded form
            p.set_payload((attachment).read())

            # encode into base64
            encoders.encode_base64(p)

            p.add_header('Content-Disposition',
                         "attachment; filename= %s" % filename)

            # attach the instance 'p' to instance 'msg'
            msg.attach(p)

            # creates SMTP session
            s = smtplib.SMTP('smtp.gmail.com', 587)

            # start TLS for security
            s.starttls()

            # Authentication
            s.login(fromaddr, "clqkejzowmyyjbbz")

            # Converts the Multipart msg into a string
            text = msg.as_string()

            # sending the mail
            s.sendmail(fromaddr, toaddr, text)

            # terminating the session
            s.quit()

            print("Email Sent Successfully!")

    def read_csvFiles(self):

        with open('DPN_Email_1st_Service_CR_by_Lagr.sync.csv', 'r', encoding='utf-8-sig') as as1st:
            csv_reader = csv.reader(as1st, delimiter=',')

            t1l = []
            for datinas1st in csv_reader:

                # print(datinas1st)
                d1 = datinas1st[0]
                d2 = datinas1st[1]
                d3 = datinas1st[2]
                d4 = datinas1st[3]
                d5 = datinas1st[4]
                d6 = datinas1st[5]
                d7 = datinas1st[6]
                d8 = datinas1st[7]
                # d9 = datinas1st[8]

                # print(d1,d2,d3,d4,d5,d6,d7)

                mkrows = (" <tr>  <td>%s</td>  <td>%s</td>  <td>%s</td>  <td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td> </tr>" %
                          (str(d1), str(d2), str(d3), str(d4), str(d5), str(d6), str(d7), str(d8)))

                t1l.append(mkrows)

                p1 = """ 
                
                 <!DOCTYPE html>
                     <html>
                     <head>
                        <title>Report</title>
                    
                        <style type="text/css">
                            * {
                      font-family: sans-serif; /* Change your font family */
                    }
                    
                    .content-table {
                      border-collapse: collapse;
                      margin: 25px 0;
                      font-size: 0.9em;
                      min-width: 400px;
                      border-radius: 5px 5px 0 0;
                      overflow: hidden;
                      box-shadow: 0 0 20px rgba(0, 0, 0, 0.15);
                    }
                    
                    .content-table thead tr {
                      background-color: blue;
                      color: #ffffff;
                      text-align: left;
                      font-weight: bold;
                    }
                    
                    .content-table th,
                    .content-table td {
                      padding: 12px 15px;
                      width: 100%;
                    }
                    
                    .content-table tbody tr {
                      border-bottom: 1px solid #dddddd;
                    }
                    
                    .content-table tbody tr:nth-of-type(even) {
                      background-color: #f3f3f3;
                    }
                    
                    .content-table tbody tr:last-of-type {
                      border-bottom: 2px solid #009879;
                    }
                    
                    .content-table tbody tr.active-row {
                      font-weight: bold;
                      color: #009879;
                    }
                     
                    
                        </style>
                     </head>
                     <body>
                        <div align="center"><img   width="200"  height="100" src='data:image/png;base64,'/></div>

                        
                        <div align="center">
                    
                      
                      """

                plst = """
                    </div>
                     </body>
                     </html>
                
                """

            pt1 = (''.join(t1l))
            # print(pt1)

            tbl1 = """<table class="content-table"> <thead><tr> <th>1st Service CR by Lagr</th> <th></th><th></th><th></th><th></th> <th></th><th></th> <th></th><th></th></tr></thead><tbody>""" + pt1 + "</tbody></table>"
            # print(tbl1)
            # this is table 1 data rows

            with open('DPN_Email_CR_by_Lagr.sync.csv', 'r', encoding='utf-8-sig') as t2:
                csv_reader1 = csv.reader(t2, delimiter=',')
                t2l = []
                for dat2 in csv_reader1:
                    # print(dat2)

                    d12 = dat2[0]
                    d22 = dat2[1]
                    d32 = dat2[2]
                    d42 = dat2[3]
                    d52 = dat2[4]
                    d62 = dat2[5]
                    d72 = dat2[6]
                    d82 = dat2[7]
                    # d92 = dat2[8]

                    mkt2 = (" <tr>  <td>%s</td>  <td>%s</td>  <td>%s</td>  <td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td> </tr>" %
                            (str(d12), str(d22), str(d32), str(d42), str(d52), str(d62), str(d72), str(d82)))

                    t2l.append(mkt2)

            # print(t2l)

            pt2 = (''.join(t2l))
            # print(pt2)
            tbl2 = """<table class="content-table"> <thead><tr> <th> CR by Lagr</th> <th></th><th></th><th></th><th></th> <th></th><th></th> <th></th><th></th></tr></thead><tbody>""" + pt2 + "</tbody></table>"
            # print(tbl2)

            # This is table 2 data rows

            with open('DPN_Email_CR_by_Reason_Lact_1.sync.csv', 'r', encoding='utf-8-sig') as t3:

                csv_reader3 = csv.reader(t3, delimiter=',')
                t3l = []
                for dat3 in csv_reader3:
                    d13 = dat3[0]
                    d23 = dat3[1]
                    d33 = dat3[2]
                    d43 = dat3[3]
                    d53 = dat3[4]
                    d63 = dat3[5]
                    d73 = dat3[6]
                    d83 = dat3[7]
                    # d93 = dat3[8]

                    mkt3 = (" <tr>  <td>%s</td>  <td>%s</td>  <td>%s</td>  <td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td> </tr>" %
                            (str(d13), str(d23), str(d33), str(d43), str(d53), str(d63), str(d73), str(d83)))

                    t3l.append(mkt3)

            pt3 = (''.join(t3l))
            # print(pt3)
            tbl3 = """<table class="content-table"> <thead><tr> <th>CR by Reason Lact_1</th> <th></th><th></th><th></th><th></th> <th></th><th></th> <th></th><th></th></tr></thead><tbody>""" + pt3 + "</tbody></table><br><br>"
            # print(tbl3)
            # this is table 3 data rows

            with open('DPN_Email_CR_by_Reason_Lact1.sync.csv', 'r', encoding='utf-8-sig') as t4:

                csv_reader4 = csv.reader(t4, delimiter=',')
                t4l = []
                for dat4 in csv_reader4:
                    d14 = dat4[0]
                    d24 = dat4[1]
                    d34 = dat4[2]
                    d44 = dat4[3]
                    d54 = dat4[4]
                    d64 = dat4[5]
                    d74 = dat4[6]
                    d84 = dat4[7]
                    # d94 = dat4[8]

                    mkt4 = (" <tr>  <td>%s</td>  <td>%s</td>  <td>%s</td>  <td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td> </tr>" %
                            (str(d14), str(d24), str(d34), str(d44), str(d54), str(d64), str(d74), str(d84)))

                    t4l.append(mkt4)

                pt4 = (''.join(t4l))
                # print(pt4)
                tbl4 = """<table class="content-table"> <thead><tr> <th>CR by Reason Lact1 </th> <th></th><th></th><th></th><th></th> <th></th><th></th> <th></th><th></th></tr></thead><tbody>""" + pt4 + "</tbody></table>"
                # print(tbl4)
                # this this table 4 data rows

                with open('DPN_Email_CR_by_Sire_Cows.sync.csv', 'r', encoding='utf-8-sig') as t5:
                    csv_reader5 = csv.reader(t5, delimiter=',')
                    t5l = []
                    for dat5 in csv_reader5:
                        d15 = dat5[0]
                        d25 = dat5[1]
                        d35 = dat5[2]
                        d45 = dat5[3]
                        d55 = dat5[4]
                        d65 = dat5[5]
                        d75 = dat5[6]
                        d85 = dat5[7]
                        # d95 = dat5[8]

                        mkt5 = (" <tr>  <td>%s</td>  <td>%s</td>  <td>%s</td>  <td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td> </tr>" %
                                (str(d15), str(d25), str(d35), str(d45), str(d55), str(d65), str(d75), str(d85)))

                        t5l.append(mkt5)

                pt5 = (''.join(t5l))
                # print(pt5)
                tbl5 = """<table class="content-table"> <thead><tr> <th>CR by Sire Cows </th> <th></th><th></th><th></th><th></th> <th></th><th></th> <th></th><th></th></tr></thead><tbody>""" + \
                    pt5 + "</tbody></table><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br>"
                # print(tbl5)
                # this is table 5 rows data

                with open('DPN_Email_CR_by_Sire_Youngstock.sync.csv', 'r', encoding='utf-8-sig') as t6:
                    csv_reader6 = csv.reader(t6, delimiter=',')
                    t6l = []
                    for dat6 in csv_reader6:
                        d16 = dat6[0]
                        d26 = dat6[1]
                        d36 = dat6[2]
                        d46 = dat6[3]
                        d56 = dat6[4]
                        d66 = dat6[5]
                        d76 = dat6[6]
                        d86 = dat6[7]
                        # d96 = dat6[8]

                        mkt6 = (" <tr>  <td>%s</td>  <td>%s</td>  <td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td> </tr>" %
                                (str(d16), str(d26), str(d36), str(d46), str(d56), str(d66), str(d76), str(d86)))

                        t6l.append(mkt6)

                pt6 = (''.join(t6l))
                # print(pt6)
                tbl6 = """<table class="content-table"> <thead><tr> <th>CR by Sire Youngstock </th> <th></th><th></th><th></th><th></th> <th></th><th></th> <th></th><th></th></tr></thead><tbody>""" + pt6 + "</tbody></table>"
                # print(tbl6)
                # this is table 6 data rows

                with open('DPN_Email_CR_by_Stud_Code_Youngstock.sync.csv', 'r', encoding='utf-8-sig') as t7:
                    csv_reader7 = csv.reader(t7, delimiter=',')
                    t7l = []
                    for dat7 in csv_reader7:
                        d17 = dat7[0]
                        d27 = dat7[1]
                        d37 = dat7[2]
                        d47 = dat7[3]
                        d57 = dat7[4]
                        d67 = dat7[5]
                        d77 = dat7[6]
                        d87 = dat7[7]
                        # d97 = dat7[8]

                        mkt7 = (" <tr>  <td>%s</td>  <td>%s</td>    <td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td> </tr>" %
                                (str(d17), str(d27), str(d37), str(d47), str(d57), str(d67), str(d77), str(d87)))

                        t7l.append(mkt7)

                pt7 = (''.join(t7l))
                # print(pt7)
                tbl7 = """<table class="content-table"> <thead><tr> <th>CR by Stud Code Youngstock </th> <th></th><th></th><th></th><th></th> <th></th><th></th> <th></th><th></th></tr></thead><tbody>""" + pt7 + "</tbody></table>"
                print(tbl7)
                # this is table 7 data rows

                with open('DPN_Email_CR_by_Tech_Lact_1.sync.csv', 'r', encoding='utf-8-sig') as t8:
                    csv_reader8 = csv.reader(t8, delimiter=',')
                    t8l = []
                    for dat8 in csv_reader8:
                        d18 = dat8[0]
                        d28 = dat8[1]
                        d38 = dat8[2]
                        d48 = dat8[3]
                        d58 = dat8[4]
                        d68 = dat8[5]
                        d78 = dat8[6]
                        d88 = dat8[7]
                        # d97 = dat7[8]

                        mkt8 = (" <tr>  <td>%s</td>  <td>%s</td>    <td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td> </tr>" %
                                (str(d18), str(d28), str(d38), str(d48), str(d58), str(d68), str(d78), str(d88)))

                        t8l.append(mkt8)

                pt8 = (''.join(t8l))
                # print(pt7)
                tbl8 = """<table class="content-table"> <thead><tr> <th>CR by Tech Lact_1 </th> <th></th><th></th><th></th><th></th> <th></th><th></th> <th></th><th></th></tr></thead><tbody>""" + pt8 + "</tbody></table>"
                print(tbl8)
                # this is table 7 data rows

                with open('DPN_Email_CR_by_Tech_Lact1.sync.csv', 'r', encoding='utf-8-sig') as t9:
                    csv_reader9 = csv.reader(t9, delimiter=',')
                    t9l = []
                    for dat9 in csv_reader9:
                        d19 = dat9[0]
                        d29 = dat9[1]
                        d39 = dat9[2]
                        d49 = dat9[3]
                        d59 = dat9[4]
                        d69 = dat9[5]
                        d79 = dat9[6]
                        d89 = dat9[7]
                        # d97 = dat7[8]

                        mkt9 = (" <tr>  <td>%s</td>  <td>%s</td>    <td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td> </tr>" %
                                (str(d19), str(d29), str(d39), str(d49), str(d59), str(d69), str(d79), str(d89)))

                        t9l.append(mkt9)

                pt9 = (''.join(t9l))
                # print(pt7)
                tbl9 = """<table class="content-table"> <thead><tr> <th>CR by Tech Lact </th> <th></th><th></th><th></th><th></th> <th></th><th></th> <th></th><th></th></tr></thead><tbody>""" + pt9 + "</tbody></table>"
                print(tbl9)
                # this is table 7 data rows

                with open('DPN_Email_CR_by_Tech_Youngstock.sync.csv', 'r', encoding='utf-8-sig') as t10:
                    csv_reader10 = csv.reader(t10, delimiter=',')
                    t10 = []
                    for dat10 in csv_reader10:
                        d10 = dat10[0]
                        d20 = dat10[1]
                        d30 = dat10[2]
                        d40 = dat10[3]
                        d50 = dat10[4]
                        d60 = dat10[5]
                        d70 = dat10[6]
                        d80 = dat10[7]
                        # d97 = dat7[8]

                        mkt10 = (" <tr>  <td>%s</td>  <td>%s</td>    <td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td> </tr>" %
                                 (str(d10), str(d20), str(d30), str(d40), str(d50), str(d60), str(d70), str(d80)))

                        t10.append(mkt10)

                pt10 = (''.join(t10))
                # print(pt7)
                tbl10 = """<table class="content-table"> <thead><tr> <th>CR by Tech Youngstock </th> <th></th><th></th><th></th><th></th> <th></th><th></th> <th></th><th></th></tr></thead><tbody>""" + pt10 + "</tbody></table>"
                print(tbl10)
                # this is table 7 data rows


                with open('DPN_Email_CR_by_Tech_Lact1.sync.csv', 'r', encoding='utf-8-sig') as t9:
                    csv_reader9 = csv.reader(t9, delimiter=',')
                    t9l = []
                    for dat9 in csv_reader9:
                        d19 = dat9[0]
                        d29 = dat9[1]
                        d39 = dat9[2]
                        d49 = dat9[3]
                        d59 = dat9[4]
                        d69 = dat9[5]
                        d79 = dat9[6]
                        d89 = dat9[7]
                        # d97 = dat7[8]

                        mkt9 = (" <tr>  <td>%s</td>  <td>%s</td>    <td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td> </tr>" %
                                (str(d19), str(d29), str(d39), str(d49), str(d59), str(d69), str(d79), str(d89)))

                        t9l.append(mkt9)

                pt9 = (''.join(t9l))
                # print(pt7)
                tbl9 = """<table class="content-table"> <thead><tr> <th>CR by Tech Lact </th> <th></th><th></th><th></th><th></th> <th></th><th></th> <th></th><th></th></tr></thead><tbody>""" + pt9 + "</tbody></table>"
                print(tbl9)
                # this is table 7 data rows

                with open('DPN_Email_CR_by_Stud_Code_Cows.sync.csv', 'r', encoding='utf-8-sig') as t11:
                    csv_reader11 = csv.reader(t11, delimiter=',')
                    t11 = []
                    for dat11 in csv_reader11:
                        d11 = dat11[0]
                        d21 = dat11[1]
                        d31 = dat11[2]
                        d41 = dat11[3]
                        d51 = dat11[4]
                        d61 = dat11[5]
                        d71 = dat11[6]
                        d81 = dat11[7]
                        # d97 = dat7[8]

                        mkt11 = (" <tr>  <td>%s</td>  <td>%s</td>    <td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td> </tr>" %
                                 (str(d11), str(d21), str(d31), str(d41), str(d51), str(d61), str(d71), str(d81)))

                        t11.append(mkt11)

                pt11 = (''.join(t11))
                # print(pt7)
                tbl11 = """<table class="content-table"> <thead><tr> <th>CR by Stud Code Cows</th> <th></th><th></th><th></th><th></th> <th></th><th></th> <th></th><th></th></tr></thead><tbody>""" + pt11 + "</tbody></table>"
                print(tbl11)
                # this is table 7 data rows

                mainpage = p1 + tbl1+tbl2+tbl3+tbl4+tbl5+tbl6+tbl7 + tbl8+tbl9+tbl10 + tbl11+ plst

                with open('test.html', 'w+') as write:
                    write.write(mainpage)
                    print("main page write successfull!!")

                    pdfkit.from_string(mainpage, 'output.pdf')

                    self.email_sender(mainpage)


if __name__ == '__main__':

    cln = Combiner()
    cln.read_csvFiles()
